...
public class SokoBot {
    ...
    // MODIFIED: Removed the static class SearchNode here, as we are now using State.java directly.
    // MODIFIED: Removed the static class HashKey here, as State.java now handles state equivalence natively.
     ...
     * and the item data representing the initial positions of boxes and the player.
    ...
    public String solveSokobanPuzzle(int width, int height, char[][] mapData, char[][] itemsData) {
        ...
        // MODIFIED: Changed SearchNode to State. Note the parameter order (playerX, playerY, crates, g, h, parent, move) matches State.java.
        State startNode = new State(startPCol, startPRow, startBoxes, 0, initialH, null, "");
        ...
        // MODIFIED: Changed PriorityQueue generic type from SearchNode to State, and closed Map generic type from HashKey to State.
        ...
        while (!openList.isEmpty()) {
            ...
            // MODIFIED: Changed from SearchNode to State.
            ...
            // MODIFIED: Replaced current.boxes with current.crates to match State.java
            ...
            // MODIFIED: Replaced current.playerRow with current.playerY, current.playerCol with current.playerX, and current.boxes with current.crates
            ...
            // MODIFIED: We no longer create a HashKey. We just apply the normalized player to the current State, which handles hashing internally.
            ...
            // MODIFIED: Changed from using the external 'key' to checking the 'current' state itself.
            ...
            // MODIFIED: Put 'current' instead of 'key'.
            ...
            // MODIFIED: current.boxes replaced with current.crates
            for (int boxIdx = 0; boxIdx < current.crates.length; boxIdx++) {
                // Get the current position of the box to be pushed, and calculate its row and column coordinates.
                // MODIFIED: current.boxes replaced with current.crates
                int boxPos = current.crates[boxIdx];
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    // MODIFIED: Replaced raw inline binarySearch with State's hasCrateAt method
                    if (current.hasCrateAt(newBoxCol, newBoxRow, width))
                        continue;
                    ...
                    // MODIFIED: current.boxes replaced with current.crates
                    int[] newBoxes = current.crates.clone();
                    ...
                    // MODIFIED: Use State constructor instead of SearchNode. Pass playerX (boxCol), playerY (boxRow), and the new crates array.
                    State neighbor = new State(
                            boxCol, boxRow, // Player ends up where the box used to be
                            newBoxes,
                            current.g + stepCost,
